    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="训练计划名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'sportClassTaskId', validatorRules.sportClassTaskId]" placeholder="请输入训练计划名称"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/edusport/sportClassTaskLeave/add",
          edit: "/edusport/sportClassTaskLeave/edit",
        }
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'sportClassTaskId','studentNo','leaveCause','startDate','endDate'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'sportClassTaskId','studentNo','leaveCause','startDate','endDate'))
      }
