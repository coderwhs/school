    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="大项" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'sportCode', validatorRules.sportCode]" placeholder="请输入大项"></a-input>
        </a-form-item>
        ...
        <a-form-item label="测试指标" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea v-decorator="['indexIds']" rows="4" placeholder="请输入测试指标"/>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        indexIds:{rules: [{ required: true, message: '请输入测试指标!' }]},
        },
        url: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'groupName','enableStatus','sportCode','eventCodes','indexIds'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'groupName','enableStatus','sportCode','eventCodes','indexIds'))
      }
